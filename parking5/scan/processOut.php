...
$checkvio = mysql_query("SELECT * FROM tbl_violist WHERE client_id = '$idno' AND client_vio_id = '1'");
	...
	if(empty ($idno)){
			...
			header("Location: ../gate1out.php?page=scan&load=scan&msg=Please Scan Your ID");
			exit;
	}
	elseif (mysql_num_rows($checkscan) == 0)
	{
			header("Location: ../gate1out.php?page=scan&load=scan&msg=You Have not Scanned Your ID yet");
			exit;
	}

elseif (mysql_num_rows($checkclient) == 0)
	{
			header("Location: ../gate1out.php?page=scan&load=scan&msg=This ID number hasn't been registered!!");
			exit;
	}
	...
	else
{
	...
	header("Location: ../gate1out.php?page=scan&load=option&cid=".$idno);
	exit;
	}	
 /*
		else if ($pref == 2)
